...
Структура данных (4 периода — today, yesterday, last_7_days, last_30_days):
...
- cnf_comments: текст комментариев к подтверждённым (CNF) заявкам
- cnx_comments: текст комментариев к отменённым (CNX) заявкам
- orq_comments: текст комментариев к запросам/предложениям (ORQ)
- pen_comments: текст комментариев к заявкам в ожидании (PEN)
- other_comments: прочие статусы
- urgent_comments: [{orderId, status, text}] срочные

СТАТУСЫ:
- CNF: Confirmed (подтверждённые, активные заказы)
- CNX: Cancelled (отменённые)
- ORQ: On Request/Quote (запросы/предложения, на согласовании)
- PEN: Pending (в ожидании, другой статус)
...
📅 Вчера (ДД/ММ/ГГГГ)
[Аналогично: CNF, CNX, ORQ, PEN — по 2-3 пункта на статус]

📊 За 7 дней (ДД/ММ/ГГГГ — ДД/ММ/ГГГГ)
[Аналогично: CNF, CNX, ORQ, PEN]

📊 За 30 дней (ДД/ММ/ГГГГ — ДД/ММ/ГГГГ)
[Аналогично: CNF, CNX, ORQ, PEN]

⚠️ Срочные (urgent) комментарии требующие внимания:
• #XXXXX [CNF/CNX/ORQ]: краткий текст
• #XXXXX [статус]: краткий текст

ВАЖНО:
- Если для статуса нет данных → напиши "(нет данных)"
- Темы CNF: запросы документов, условия, платежи, даты, переоформления, инфо
- Причины CNX: передумал, цена, нет мест, переадресован, техпроблемы, форс-мажор
- Статусы ORQ/PEN: этапы согласования, задержки, блокирующие проблемы
- Срочные — показывай из периода "yesterday", до 5 штук
