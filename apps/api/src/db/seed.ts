...
async function main() {
  ...
  for (const src of sources) {
    ...
    // Default schedules
    const defaultSchedules: Record<string, { name: string; description: string; cron: string; weekendCron?: string; periodType: 'daily' | 'weekly' | 'monthly' }[]> = {
      gto:      [
        { name: 'Daily Sales Report',   description: 'Sales metrics for yesterday',        cron: '0 8 * * *',   weekendCron: '0 10 * * 0,6', periodType: 'daily'   },
      ...
      gto_comments: [
        { name: 'Daily Comments Report',   description: 'AI analysis of order comments for today and yesterday', cron: '0 8 * * *', weekendCron: '0 10 * * 0,6', periodType: 'daily'   },
      ...
      ga4:      [
        { name: 'Daily Traffic Report',  description: 'Web traffic metrics for yesterday',  cron: '0 8 * * *',   weekendCron: '0 10 * * 0,6', periodType: 'daily'   },
        { name: 'Weekly Traffic Summary',description: 'Aggregated traffic for the past week',cron: '0 9 * * 1',  periodType: 'weekly'  },
      ],
      redmine:  [
        { name: 'Daily Issues Report',   description: 'Issue activity for yesterday',        cron: '0 8 * * *',   weekendCron: '0 10 * * 0,6', periodType: 'daily'   },
      ...
      youtrack_progress: [
        { name: 'Daily Progress Report', description: 'Yesterday progress after the daily standup', cron: '0 8 * * *', weekendCron: '0 10 * * 0,6', periodType: 'daily' },
      ],
    };
      ...
      if (!existing) {
        await prisma.reportSchedule.create({
          data: {
            ...
            weekendCronExpression: sch.weekendCron ?? null,
            periodType: sch.periodType,
            isEnabled: false,
          },
        });
      }
...
💶 Выручка по CNF: X EUR
Прибыль по CNF: X EUR (X%)
💼 Средний чек по CNF: X EUR
...
- Топ агент: section1_yesterday.top_agents_by_orders[0] (по всем заявкам дня, исключая тестового агента "GTO for Test-Goodwin")
...
- Заказы тестового агента "GTO for Test-Goodwin" не учитывать нигде в отчёте: ни в статусах, ни в туристах, ни в деньгах, ни в рейтингах.
...
- Для денежных строк daily/7-day явно подписывай, что это "по CNF":
  ...
  "Прибыль по CNF: ..."
  "💼 Средний чек по CNF: ..."
- Сразу после строки "💼 Средний чек по CNF: ..." добавляй пояснение:
  "Все денежные показатели приведены к EUR."
- Штуки: целые числа, без разделителя если < 10000.
- Блок направлений оформляй заголовком "---🌍 Направления---", затем топ-3 страны, каждая на новой строке в формате "🏳Страна X зак / X тур" (orders и tourists).
- Блок продуктов оформляй заголовком "---📦 Продукты---", с пустой строкой между блоками направлений и продуктов. Каждая строка отдельно. emoji 🏨 Пакет, 🏩 Отель, ✈️ Перелёт; показывай orders и tourists.
- Между логическими блоками Telegram-отчёта добавляй пустую строку: между "---🌍 Направления---" и "---📦 Продукты---", между "🔴 Отрицательная маржа" и "🔮 Старт Ближ. 7 дней", а также перед "Старт ближ. 30 дней".
- В "🔮 Старт Ближ. 7 дней" и "Старт ближ. 30 дней" summary разносить на 4 строки после заголовка:
  "X заказов,"
  "X туристов,"
  "GMV: X EUR,"
  "Gross profit: X EUR (X%)"
...
- АНОМАЛИИ — выводить ПОСЛЕ блока поставщиков в каждом дневном разделе:
  ...
  • Блок "⚠️ Прочие аномалии" не выводить вообще.
- В блоках "🔮" страны всегда сортировать по количеству туристов по убыванию.
...
- people_activity: покажи по людям, кто что сделал за день. Для каждого человека перечисли 1-3 самых важных действия и обязательно укажи названия задач.
- people_activity: НЕ склеивай несколько задач человека в одну строку. Формат должен позволять вывести: сначала имя человека на отдельной строке, затем по одной строке на каждую задачу с описанием действия/изменения по этой задаче.
- main_problems: это должен быть список конкретных проблем с деталями: по какой задаче, как называется задача, что именно сломано/зависло, кто упоминал проблему, что сейчас мешает.
...
- Если у человека есть задачи в contributor_activity.tasks, используй и ключ, и summary задачи в people_activity и telegram_message.
- В блоке "кто что сделал" не ограничивайся только именем человека: показывай конкретные задачи, например "Denys Skorov — gw_main-1499 Filters improvement".
- В telegram_message блок "Кто что сделал" оформляй строго так:
  Anna Tinna:
  gw_main-592 — Название задачи — что именно добавила/изменила
  gw_main-1622 — Название задачи — что именно протестировала/уточнила

  Ruslan Nikitenko:
  gw_main-1431 — Название задачи — что именно добавил/оценил/уточнил
  gw_main-1433 — Название задачи — что именно сделал
- По каждой задаче обязательно указывай не только ключ и название, но и краткое действие: комментарий, оценка, вопрос, уточнение сроков, тест-кейс, предложение решения, подтверждение мерджа и т.д.
...
Верни ТОЛЬКО валидный JSON:
{
  ...
  "people_activity": [
    {
      ...
      "tasks": ["KEY-1 — Название задачи", "KEY-2 — Название задачи"],
  ...
  "telegram_message": "Markdown-отчёт для Telegram, максимум 3500 символов, обязательно со структурой: 1) краткий итог, 2) кто что сделал с ключами и названиями задач, 3) что сдвинулось по задачам, 4) основные проблемы с деталями и названиями задач, 5) следующие шаги"
}`,
      },
    };
